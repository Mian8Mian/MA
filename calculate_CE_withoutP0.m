...
for n=1:ordofe
    ...
    while ~isempty(occurrences_n)
        ...
        if ~isempty(occnr)     
            ...
            if Hy(j)~=0
                nm_in_n_spot=find(buffermat_n==occurrences_n(i,1),1,'first');
                comb_nm_in_n=combination_n(nm_in_n_spot).combination(1:n);
                found=0;
                g=1;
                while found==0
                    if combination_nm(g).combination==comb_nm_in_n
                        nmspot=combination_nm(g).position;
                        found=1;
                    end
                    g=g+1;
                end
                pnmspot=find(occurrences_nm(:,1)==nmspot);
                H_ALL(n+1)=H_ALL(n+1)+Hy(j)*p_nm(pnmspot,2);
            end
